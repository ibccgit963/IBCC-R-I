    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
        <div class="px-6 py-4 bg-[#00583A] border-b border-[#004028]">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-white">Ministry/Department Dispatch</h1>
                    <p class="text-sm text-white mt-1">
                        @if(Auth::user()->role->slug === 'officer')
                            Submit ministry/department dispatch requests to R&amp;I for processing.
                        @else
                            Manage all ministry/department dispatches and their details.
                        @endif
                    </p>
                </div>
            </div>
        ...
        <div class="mb-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
            <div class="mb-4 border-b border-gray-200 pb-2">
                @if(Auth::user()->role->slug === 'officer')
                    <h2 class="text-lg font-bold text-gray-800">Submit Ministry/Department Dispatch Request</h2>
                    <p class="text-sm text-gray-500">Fill in the details and submit to R&amp;I for dispatch.</p>
                @else
                    <h2 class="text-lg font-bold text-gray-800">Create Ministry/Department Dispatch</h2>
            ...
            <form method="POST" action="{{ route('dispatches.store') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-4">
                        <div>
                            <label for="applicant_name" class="block font-medium text-sm text-gray-700">Ministry / Department Name</label>
                            <input id="applicant_name" type="text" name="applicant_name" value="{{ old('applicant_name') }}" required placeholder="Enter Ministry or Department Name"
                        ...
                        <div>
                            <label for="case_number" class="block font-medium text-sm text-gray-700">Reference / File Number</label>
                            <input id="case_number" type="text" name="case_number" value="{{ old('case_number') }}" required placeholder="Enter Reference or File Number"
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="dispatched_from" class="block font-medium text-sm text-gray-700">From (Department/Office)</label>
                            @if(Auth::user()->role->slug === 'officer')
                            ...
                            @else
                                <input id="dispatched_from" type="text" name="dispatched_from" value="{{ old('dispatched_from') }}" required placeholder="Enter From Department or Office"
                        ...
                        <div>
                            <label for="father_name" class="block font-medium text-sm text-gray-700">Subject / Description (Optional)</label>
                            <input id="father_name" type="text" name="father_name" value="{{ old('father_name') }}" placeholder="Enter subject or description"
                ...
                <div class="flex items-center justify-end mt-4 pt-4 border-t border-gray-200">
                    <button type="submit" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 border border-transparent rounded-lg font-semibold text-xs text-white uppercase tracking-widest shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all">
                        ...
                        @if(Auth::user()->role->slug === 'officer') Submit Request @else Create Dispatch @endif
                    </button>
                </div>
            </form>
        </div>
        ...
        @endif
        @if(session('error'))
            <div class="mx-6 mb-4 bg-red-50 border-l-4 border-red-500 text-red-700 p-4 rounded-r shadow-sm text-sm">{{ session('error') }}</div>
        @endif
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-[#007bff] text-white">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">Ministry/Department</th>
                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">Reference No.</th>
                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">Tracking ID</th>
                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">From</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($dispatches as $dispatch)
                        <tr class="bg-gray-50 hover:bg-gray-100 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">{{ $dispatch->applicant_name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $dispatch->case_number }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-purple-600">{{ $dispatch->tracking_id ?? '—' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $dispatch->dispatched_from }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $dispatch->father_name ?? '—' }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                @if(in_array(Auth::user()->role->slug, ['staff-user', 'center-admin', 'super-admin']))
                                    ...
                                    @endif
                                @else
                                    <span class="text-xs text-gray-400">{{ $dispatch->created_at->format('d M Y') }}</span>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ Auth::user()->role->slug !== 'officer' ? 9 : 8 }}" class="px-6 py-10 text-center text-gray-500">
                                <div class="flex flex-col items-center justify-center">
                                    <i class="fa-solid fa-file-import text-4xl text-gray-300 mb-3"></i>
                                    <p class="text-lg font-medium">No ministry/department dispatches found.</p>
                                    <p class="text-sm text-gray-400">
                                        @if(Auth::user()->role->slug === 'officer') Submit a request using the form above.
                                        @else Create a new dispatch using the form above. @endif
                                    </p>
                                </div>
                            </td>
                        </tr>
