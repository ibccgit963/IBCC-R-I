...
<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-4">
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
        <div class="px-6 py-4 bg-[#00583A] border-b border-[#004028]">
            ...
            <p class="text-sm text-white mt-1">Modify the details for this department.</p>
        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input id="name" type="text"
                                ...
                                name="name" value="{{ old('name', $department->name) }}" required autofocus placeholder="Enter department name">
                            @error('name')
                            ...
                            <select id="center_id"
                                ...
                                @foreach ($centers as $center)
                                    <option value="{{ $center->id }}"
                                        {{ old('center_id', $department->center_id) == $center->id ? 'selected' : '' }}>
                                        {{ $center->name }}</option>
                                @endforeach
                            </select>
                            @error('center_id')
