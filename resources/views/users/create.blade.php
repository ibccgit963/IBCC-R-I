...
<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-4">
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
        <div class="px-6 py-4 bg-[#00583A] border-b border-[#004028]">
            ...
            <p class="text-sm text-white mt-1">Fill in the details to register a new system user.</p>
        </div>
            ...
            <form method="POST" action="{{ route('users.store') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div class="space-y-3">
                        ...
                        <div>
                            ...
                            <input id="name" type="text" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('name') border-red-500 @enderror" name="name" value="{{ old('name') }}" required autofocus>
                            @error('name')
                        ...
                        <div>
                            ...
                            <input id="email" type="email" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('email') border-red-500 @enderror" name="email" value="{{ old('email') }}" required>
                            @error('email')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <!-- Employee ID -->
                        <div>
                            <label for="employee_id" class="block font-medium text-sm text-gray-700">{{ __('Employee ID') }}</label>
                            <input id="employee_id" type="text" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('employee_id') border-red-500 @enderror" name="employee_id" value="{{ old('employee_id') }}" required>
                            @error('employee_id')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div>
                            ...
                            @if (Auth::user()->role->slug === 'super-admin')
                                <select id="center_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('center_id') border-red-500 @enderror" name="center_id" required>
                            ...
                            @else
                                ...
                                <input type="text" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm bg-gray-100 cursor-not-allowed sm:text-sm" value="{{ Auth::user()->center->name }}" disabled>
                            @endif
                            @error('center_id')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            @endphp
                            <select id="role_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('role_id') border-red-500 @enderror" name="role_id" required>
                        ...
                        <div id="focal_person_container" class="hidden mt-4">
                            <label class="flex items-center">
                                <input type="checkbox" name="is_focal_person" id="is_focal_person" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('is_focal_person') ? 'checked' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                    ...
                    <div>
                        ...
                        <input id="password" type="password" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('password') border-red-500 @enderror" name="password" required>
                        @error('password')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input id="password-confirm" type="password" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" name="password_confirmation" required>
                    </div>
                </div>

                <div class="flex items-center justify-end mt-4 pt-3 border-t border-gray-200 space-x-4">
                    <a href="{{ route('users.index') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:text-gray-500 transition">
                        Cancel
                    </a>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (centerEl.tagName === 'SELECT') {
        ...
        // On page load, if a center is already selected (e.g. after validation error), fetch its departments
        if (centerEl.value) {
