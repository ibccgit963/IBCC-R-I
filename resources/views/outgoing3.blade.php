    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
        <div class="px-6 py-4 bg-[#00583A] border-b border-[#004028]">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-white">Sub-office Dispatch</h1>
                    <p class="text-sm text-white mt-1">
                        @if(Auth::user()->role->slug === 'officer')
                            Submit sub-office dispatch requests to R&amp;I for processing.
                        @else
                            Manage all sub-office dispatches and their details.
                        @endif
                    </p>
                </div>
            </div>
        ...
        <div class="mb-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
            <div class="mb-4 border-b border-gray-200 pb-2">
                @if(Auth::user()->role->slug === 'officer')
                    <h2 class="text-lg font-bold text-gray-800">Submit Sub-office Dispatch Request</h2>
                    <p class="text-sm text-gray-500">Fill in the details and submit to R&amp;I for dispatch.</p>
                @else
                    <h2 class="text-lg font-bold text-gray-800">Create Sub-office Dispatch</h2>
            ...
            <form method="POST" action="{{ route('dispatches.store') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-4">
                        <div>
                            <label for="applicant_name" class="block font-medium text-sm text-gray-700">Recipient Name</label>
                            <input id="applicant_name" type="text" name="applicant_name" value="{{ old('applicant_name') }}" required placeholder="Enter Recipient Name"
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="dispatched_from" class="block font-medium text-sm text-gray-700">Sub-office / From</label>
                            @if(Auth::user()->role->slug === 'officer')
                            ...
                            @else
                                <input id="dispatched_from" type="text" name="dispatched_from" value="{{ old('dispatched_from') }}" required placeholder="Enter Sub-office or Office Name"
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-[#007bff] text-white">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">Recipient Name</th>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">Sub-office / From</th>
                        ...
                        <tr>
                            <td colspan="{{ Auth::user()->role->slug !== 'officer' ? 9 : 8 }}" class="px-6 py-10 text-center text-gray-500">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-lg font-medium">No sub-office dispatches found.</p>
