    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
        <div class="px-6 py-4 bg-[#00583A] border-b border-[#004028]">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-white">Ministry/Department Receive</h1>
                    <p class="text-sm text-white mt-1">Manage all ministry/department incoming couriers and their details.</p>
                </div>
            </div>
            </div>
            ...
            <div class="mb-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
                <div class="mb-4 border-b border-gray-200 pb-2">
                    <h2 class="text-lg font-bold text-gray-800">Register New Ministry/Department Receive</h2>
                    <p class="text-sm text-gray-500">Enter the details of the ministry/department incoming courier package.</p>
                </div>
                ...
                <form method="POST" action="{{ route('couriers.store') }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <!-- Right Column -->
                        <div class="space-y-4">
                            <!-- Ministry/Department (Autocomplete) -->
                            <div>
                                <label for="ministry_department" class="block font-medium text-sm text-gray-700">{{ __('Ministry/Department') }}</label>
                                <input id="ministry_department" type="text" list="ministry_list" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('ministry_department') border-red-500 @enderror" name="ministry_department" value="{{ old('ministry_department') }}" placeholder="Enter Ministry/Department">
                                <datalist id="ministry_list">
                                    @foreach($ministries as $ministry)
                                        <option value="{{ $ministry }}">
                                    @endforeach
                                </datalist>
                                @error('ministry_department')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($couriers as $courier)
                            <tr class="bg-gray-50 hover:bg-gray-100 transition-colors duration-200 border-b border-gray-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                    {{ $courier->ministry_department ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                    {{ $courier->center->name }}
                                </td>
