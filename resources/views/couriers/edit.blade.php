        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
            <div class="p-5 bg-white border-b border-gray-200">
                <div class="mb-2 border-b border-gray-200 pb-2">
                    <h1 class="text-2xl font-bold text-gray-800">Edit Incoming Courier</h1>
                    <p class="text-sm text-gray-500 mt-1">Update the details of the incoming courier package.</p>
                </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                ...
                                <input id="tracking_id" type="text"
                                    ...
                                    name="tracking_id" value="{{ old('tracking_id', $courier->tracking_id) }}" required autofocus>
                                @error('tracking_id')
                            ...
                            <div>
                                ...
                                <input id="courier_company" type="text"
                                    ...
                                    name="courier_company" value="{{ old('courier_company', $courier->courier_company) }}"
                            ...
                            <div>
                                ...
                                <input id="sender_name" type="text"
                                    ...
                                    name="sender_name" value="{{ old('sender_name', $courier->sender_name) }}" required>
                                @error('sender_name')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <select id="type" name="type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('type') border-red-500 @enderror" required>
                                    <option value="applicant" {{ old('type', $courier->type) == 'applicant' ? 'selected' : '' }}>Applicant</option>
                                    <option value="internal" {{ old('type', $courier->type) == 'internal' ? 'selected' : '' }}>Internal</option>
                                    <option value="sub_office" {{ old('type', $courier->type) == 'sub_office' ? 'selected' : '' }}>Sub Office</option>
                                    <option value="ministry" {{ old('type', $courier->type) == 'ministry' ? 'selected' : '' }}>Ministry/Department</option>
                                </select>
                                @error('type')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                ...
                                <input id="sender_cnic" type="text"
                                    ...
                                    name="sender_cnic" value="{{ old('sender_cnic', $courier->sender_cnic) }}">
                                @error('sender_cnic')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input id="sender_contact" type="text"
                                    ...
                                    name="sender_contact" value="{{ old('sender_contact', $courier->sender_contact) }}">
                            ...
                            <div>
                                ...
                                <select id="department_id"
                                    ...
                                    @php
                                        $departments = \App\Models\Department::where('center_id', $courier->center_id)
                                            ->whereHas('users', fn($q) => $q->whereHas('role', fn($r) => $r->where('slug', 'officer')))
                                            ->get();
                                    @endphp
                                    @foreach ($departments as $department)
                                        <option value="{{ $department->id }}"
                                            {{ old('department_id', $courier->department_id) == $department->id ? 'selected' : '' }}>
                                            {{ $department->name }}
                                        </option>
                                    @endforeach
                                </select>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const oldAssignedUserId = "{{ old('assigned_user_id', $courier->assigned_user_id) }}";
                ...
                // Trigger change event if department is already selected (e.g. old input or current DB value)
