                                    ...
                                    @if(Auth::user()->role->slug === 'officer' && $courier->status === 'transferred')
                                        @if($courier->transfers->isNotEmpty())
                                            <form action="{{ route('courier-transfers.markReceived', $courier->transfers->last()->id) }}" method="POST" class="inline-block">
                                                @csrf
                                                <button type="submit" class="inline-flex items-center px-3 py-1 bg-blue-600 text-white text-xs font-bold rounded hover:bg-blue-700 transition-colors shadow-sm" title="Mark as Received">
                                        ...
                                        <!-- Revert Button triggers Alpine Modal -->
