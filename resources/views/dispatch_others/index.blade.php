    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
        <div class="px-6 py-4 bg-[#00583A] border-b border-[#004028]">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-white">Sub-offices Dispatch</h1>
                    <p class="text-sm text-white mt-1">Manage sub-offices dispatch logs and details.</p>
                </div>
                <div>
                    <a href="{{ route('dispatch-others.export', request()->all()) }}" class="inline-flex items-center px-4 py-2 bg-white text-[#00583A] rounded-lg font-semibold text-sm hover:bg-gray-100 transition-colors shadow-sm">
        ...
        <div class="mb-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
            <div class="mb-4 border-b border-gray-200 pb-2">
                <h2 class="text-lg font-bold text-gray-800">Add New Sub-offices Dispatch</h2>
                <p class="text-sm text-gray-500">Enter details and press Enter to save.</p>
            </div>

            <form method="POST" action="{{ route('dispatch-others.store') }}" id="dispatch-form">
                @csrf
        ...
        <!-- Filters -->
        <div class="px-6 mb-4">
            <form method="GET" action="{{ route('dispatch-others.index') }}" class="flex flex-wrap gap-4 items-end">
                <div>
                ...
                <a href="{{ route('dispatch-others.index') }}" class="inline-flex items-center px-4 py-2 text-gray-500 hover:text-gray-700 h-[38px]">
                    Reset
                </a>
            </form>
        </div>
